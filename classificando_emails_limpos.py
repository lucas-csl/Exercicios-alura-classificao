...
texto_puro = df["email"]
frases = texto_puro.str.lower()
texto_quebrado = [nltk.tokenize.word_tokenize(frase) for frase in frases]
...
for lista in texto_quebrado:
	valida = [stemmer.stem(palavra) for palavra in lista if palavra not in stopwords and len (palavra) > 2]
	dicionario.update (valida)
